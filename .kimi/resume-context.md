...
| 分支        | `main`                                                                          |
| 最新提交    | `122bec1 docs: 恢复指令统一为 /resume dev xinqu`                                |
...
- **主题**：等待用户下一步指令。
- **状态**：
  - 审批中心“驳回到指定阶段并指定具体处理人”功能已完成并提交。
  - 恢复开发上下文机制已建立：支持 `/resume dev xinqu` 读取本快照并恢复环境。
  - 前后端开发服务已重启并尝试打开页面。
- **待办**：根据用户选择继续推进，例如审批中心进一步完善、页面布局修复、复用性改造、补全其他应用功能等。

---

## 最近的变更（按时间倒序）

1. `122bec1` — 恢复指令统一为 `/resume dev xinqu`
2. `5d9cb22` — 新增开发上下文快照与恢复指令，更新 `resume-dev` skill
3. `1d8dadc` — 修复驳回阶段选择按钮的模板表达式语法错误
4. `75942e9` — 驳回支持指定具体处理人
5. `d38de3e` — 本地审批模式支持驳回到指定节点
6. 更早：审批中心重构为邮件式发起、图形化流程设计器、本地/Camunda 双模式等。

---

## 开发服务状态（本机运行时）

| 服务         | 命令                   | 默认地址                    | 当前任务 ID     | 备注                          |
| ------------ | ---------------------- | --------------------------- | --------------- | ----------------------------- |
| 后端开发服务 | `npm run dev:server`   | `http://localhost:3001/api` | `bash-dpeldq0i` | 端口因 3000 占用已调整为 3001 |
| 前端开发服务 | `npm run dev:web`      | `http://localhost:5173`     | `bash-zlqu1wr4` | Vite 代理 `/api/v1` 到 3001   |
