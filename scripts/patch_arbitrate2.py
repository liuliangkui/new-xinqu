...
# ---------- 2. replace grid/list inside view-intentions ----------
view_start = text.find('<section class="view" id="view-intentions">')
if view_start == -1:
    raise SystemExit('view-intentions not found')
grid_start = text.find('<!-- Grid view -->', view_start)
list_start = text.find('<!-- List view -->', view_start)
pagination_start = text.find('<!-- Pagination -->', list_start)
if grid_start == -1 or list_start == -1 or pagination_start == -1:
    raise SystemExit('grid/list/pagination markers not found in view-intentions')
...
list_old = text[list_start:pagination_start]
rows_html = ''
for it in intentions:
    ...
    if it['conflict_status'] == 'completed' or it['conflict_status'] == 'rejected':
        op = f'<button class="btn btn-primary text-xs" onclick="event.stopPropagation();openArbitrationLetter(\'INT-2026073005\')">查看裁决书</button>'
...
print('Grid/list replaced in view-intentions')

# ---------- 3. insert conflict module in detail modal ----------
...
'''
modal_start = text.find('id="intention-modal-overlay"')
marker_protect = '                <!-- 保护期与汇报管理 -->\n'
protect_pos = text.find(marker_protect, modal_start)
if protect_pos != -1:
    text = text[:protect_pos] + conflict_module_html + text[protect_pos:]
else:
    print('protect marker not found')

# ---------- 4. update stageLabel map ----------
...
# ---------- 5. update openIntentionModal lifecycle render & todo ----------
...
lifecycle_extra = """      document.getElementById('modal-intention-lifecycle').innerHTML = renderLifecycleStageBar(lifeStage, lifeDwell, lifeOverdue);
      ...
      const oldBanner = lc.parentElement.querySelector('.conflict-lifecycle-banner');
...
# ---------- 6. add renderIntentionConflict helper ----------
render_conflict_js = """
...
# ---------- 7. add arbitration modals before </body> ----------
modals_html = '''
  <!-- ===== 撞单裁决弹窗 ===== -->
...
# ---------- 8. add JS functions before </script> ----------
arbitrate_js = """
...
# ---------- 9. update approval center records ----------
