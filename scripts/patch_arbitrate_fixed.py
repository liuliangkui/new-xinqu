...
# ---------- 1. add CSS ----------
css_addition = '''    .badge-yellow { background: var(--warning-bg); color: #FF8800; }
    .badge-purple { background: rgba(124,58,237,0.12); color: #7c3aed; }
    html[data-theme="dark"] .badge-yellow { background: var(--warning-bg); color: #FACC15; }
    ...
    .lifecycle-stage { flex: 1; min-width: 4.5rem; text-align: center; padding: 8px 6px; border-radius: 10px; border: 1px solid var(--line); background: var(--panel); color: var(--sub); font-size: 0.786rem; }
    .lifecycle-stage.current { background: var(--primary-light); color: var(--primary); border-color: var(--primary); font-weight: 600; }
    .lifecycle-stage.completed { background: var(--success-bg); color: var(--success); border-color: var(--success); }
'''
marker_css = '    .badge-gray { background: var(--gray-bg); color: var(--sub); }\n'
...
# ---------- 2. replace grid cards inside view-intentions ----------
cards_html = ''
for it in intentions:
...
grid_pattern = r'(<!-- Grid view -->\s*<div id="intention-grid" class="card-grid mb-4">).*?(</div>\s*)(<!-- List view -->)'
grid_match = re.search(grid_pattern, text, flags=re.DOTALL)
if grid_match:
    text = text[:grid_match.start()] + grid_match.group(1) + '\n' + cards_html + grid_match.group(2) + grid_match.group(3) + text[grid_match.end():]
    print('Grid cards replaced')
else:
    print('Grid pattern not found')

# ---------- 3. replace list rows inside view-intentions ----------
rows_html = ''
for it in intentions:
...
list_pattern = r'(<!-- List view -->\s*<div id="intention-list" class="card mb-4 hidden">\s*<table class="data-table">\s*<thead>.*?</thead>\s*<tbody>\s*).*?(\s*</tbody>\s*</table>\s*</div>)\s*(<div class="flex items-center justify-between">\s*<div class="text-sub text-sm">共 186 条 · 每页 12 条</div>)'
list_match = re.search(list_pattern, text, flags=re.DOTALL)
if list_match:
    text = text[:list_match.start()] + list_new + '\n\n          ' + list_match.group(3) + text[list_match.end():]
    print('List rows replaced')
else:
    print('List pattern not found')

# ---------- 4. insert lifecycle + conflict module in detail modal ----------
modal_marker = '                <!-- 主体内容：左侧 2/3 分组信息+附件，右侧 1/3 当前待办+时间线 -->\n'
if modal_marker in text:
    lifecycle_conflict_html = '''                <!-- 生命周期 -->
                <div class="mb-4">
                  <div class="text-xs text-sub mb-2">意向生命周期</div>
                  <div id="modal-intention-lifecycle" class="flex items-center gap-2 overflow-x-auto py-2"></div>
                </div>
...
'''
    text = text.replace(modal_marker, lifecycle_conflict_html + modal_marker)
    print('Lifecycle + conflict module inserted')
else:
    print('Detail modal marker not found')
...
if old_stage_map in text:
    ...
    print('Stage map updated')
else:
    print('Stage map marker not found')

# ---------- 6. add lifecycle helpers before openIntentionModal ----------
lifecycle_js = """    function getLifecycleStages() {
      return ['线索', '意向', '方案', '装机', '试剂供应'];
    }

    function renderLifecycleStageBar(stage, dwell, overdue) {
      const stages = getLifecycleStages();
      const idx = stages.indexOf(stage);
      return stages.map((label, i) => {
        let cls = 'lifecycle-stage';
        if (i === idx) cls += ' current';
        else if (i < idx && idx !== -1) cls += ' completed';
        let info = '';
        if (i === idx) info = '<div class="text-xs mt-1 ' + (overdue ? 'text-danger' : 'text-sub') + '">已停留 ' + (dwell || 0) + ' 天</div>';
        return '<div class="' + cls + '"><div class="font-medium">' + label + '</div>' + info + '</div>';
      }).join('');
    }

"""
marker_open_intention = '    function openIntentionModal(el) {\n'
if marker_open_intention in text:
    text = text.replace(marker_open_intention, lifecycle_js + marker_open_intention)
    print('Lifecycle helpers added')
else:
    print('openIntentionModal marker not found')

# ---------- 7. update openIntentionModal lifecycle render & todo ----------
progress_line = "      document.getElementById('modal-intention-progress').innerHTML = renderIntentionProgressSteps(d.currentStage, d.status);\n"
progress_extra = """      document.getElementById('modal-intention-progress').innerHTML = renderIntentionProgressSteps(d.currentStage, d.status);

      // 生命周期条与撞单冲突联动
      const lifeStage = d.lifeStage || '-';
      const lifeStageIndex = parseInt(d.lifeStageIndex, 10) || 0;
      const lifeDwell = d.lifeDwell || '0';
      const lifeOverdue = d.lifeOverdue === 'true';
      document.getElementById('modal-intention-lifecycle').innerHTML = renderLifecycleStageBar(lifeStage, lifeDwell, lifeOverdue);
      const lc = document.getElementById('modal-intention-lifecycle');
      lc.parentElement.querySelectorAll('.conflict-lifecycle-banner').forEach(b => b.remove());
      if (d.conflictStatus && d.conflictStatus !== 'none') {
        const stages = lc.querySelectorAll('.lifecycle-stage');
        stages.forEach((s, i) => { if (i === lifeStageIndex) s.classList.add('conflict-lifecycle-current'); });
        const banner = document.createElement('div');
        banner.className = 'conflict-lifecycle-banner mt-2 flex items-center gap-2 text-sm text-warning font-medium';
        banner.innerHTML = '<svg class="icon icon-sm"><use href="#icon-warning"/></svg><span>撞单裁决中</span>';
        lc.parentElement.appendChild(banner);
        renderIntentionConflict(d);
      } else {
        document.getElementById('modal-intention-conflict').classList.add('hidden');
      }
"""
if progress_line in text:
    text = text.replace(progress_line, progress_extra)
    print('Lifecycle render added')
else:
    print('progress render line not found')
...
if old_conflict_todo in text:
    ...
    print('Conflict todo updated')
else:
    print('conflict todo block not found')

# ---------- 8. add renderIntentionConflict helper ----------
render_conflict_js = """
...
if marker_create_intention in text:
    ...
    print('renderIntentionConflict helper added')
else:
    print('create intention marker not found')

# ---------- 9. add arbitration modals before </body> ----------
modals_html = '''
  <!-- ===== 撞单裁决弹窗 ===== -->
...
if marker_body_end in text:
    ...
    print('Arbitration modals added')
else:
    print('body end marker not found')

# ---------- 10. add JS functions before </script> ----------
arbitrate_js = """
...
if marker_script_end in text:
    ...
    print('Arbitration JS functions added')
else:
    print('script end marker not found')

# ---------- 11. update approval center records ----------
...
if old_approval_total in text:
    ...
    print('Approval total updated')
else:
    print('approval total not found')

approval_view_start = text.find('<section class="view" id="view-approvals">')
approval_space_marker = '<div class="space-y-3 mt-3">\n'
approval_space_pos = text.find(approval_space_marker, approval_view_start)
if approval_space_pos != -1:
    insert_pos = approval_space_pos + len(approval_space_marker)
    new_cards = '''              <div class="app-card card p-4">
              ...
              <div class="app-card card p-4">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  <div class="flex items-start gap-4">
                    <div class="w-12 h-12 rounded-xl bg-info-bg text-info flex items-center justify-center flex-shrink-0"><svg class="icon icon-xl"><use href="#icon-warning"/></svg></div>
                    <div>
...
'''
    text = text[:insert_pos] + new_cards + text[insert_pos:]
    print('Approval cards inserted')
else:
    print('approval space marker not found')
