...
import type { PerformanceQueryDto } from './dto/performance-query.dto'
...
export class PerformanceService {
  ...
  getOverview(user: CurrentUser, query: PerformanceQueryDto) {
    const { period = 'month', indicator = 'revenue' } = query
    ...
    const underperformCount = Math.floor(Math.random() * 8) + 2
  ...
  findAll(user: CurrentUser, query: PerformanceQueryDto) {
    const { page = 1, pageSize = 12, keyword, tabType = 'team', period = 'month', indicator = 'revenue' } = query

    let source: PerformanceItem[] = []
    const baseMultiplier = indicator === 'quantity' ? 1 : indicator === 'collection' ? 0.85 : 1
    const baseTarget = (period === 'year' ? 70000000 : period === 'quarter' ? 18000000 : 6000000) * baseMultiplier

    switch (tabType) {
      case 'team':
        source = this.teams.map((t) => this.buildItem(t.id, t.name, baseTarget, { ownerName: t.ownerName }))
        break
      case 'region':
        source = this.regions.map((r) => this.buildItem(r.code, r.name, baseTarget, { regionName: r.name }))
        break
      case 'product':
        source = this.products.map((p) => this.buildItem(p.id, p.name, baseTarget, { productLine: p.productLine }))
        break
      case 'channel':
        source = this.channels.map((c) => this.buildItem(c.id, c.name, baseTarget, { regionName: c.regionName }))
        break
      case 'my':
      default:
        source = [this.buildItem(user.userId, '我的绩效', baseTarget, { ownerName: '本人' })]
        break
    }
